            ...
            if node.name in node_name_to_shard_id and prev_shard_id < node_name_to_shard_id[node.name]:
                ...
                with new_graph.inserting_after(prev_node):
                    ...
                    if isinstance(nodes_per_shard[prev_shard_id][prev_node.name], tuple):
                        graph_outputs = nodes_per_shard[prev_shard_id][prev_node.name] + \
                            tuple([nodes_per_shard[prev_shard_id][i] for i in outputs if i != prev_node.name])
                    else:
                        graph_outputs = tuple([nodes_per_shard[prev_shard_id][prev_node.name]] + \
